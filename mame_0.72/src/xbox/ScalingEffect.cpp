...
void ScalingEffect::KillThis()
{
	...
	m_VideoInputHandle = 0;
	m_VideoSizeHandle = 0;
	m_TextureSizeHandle = 0;
	m_OutputSizeHandle = 0;
}
...
HRESULT ScalingEffect::LoadEffect(const TCHAR *filename)
{
    ...
    HRESULT hr = D3DXCreateEffectCompilerFromFile(filename, NULL, NULL,D3DXSHADER_DEBUG,
...
HRESULT ScalingEffect::SetVideoInputs(int iWidth, int iHeight, int vpWidth, int vpHeight, int fc)
{

	HRESULT hr;

	D3DXVECTOR2 x = D3DXVECTOR2(iWidth,iHeight);
	D3DXVECTOR2 y = D3DXVECTOR2(vpWidth,vpHeight);
	D3DXVECTOR2 frame_count = D3DXVECTOR2(fc,fc);

	hr = m_pEffect->SetFloatArray(m_VideoSizeHandle,(float*)x,2);
	hr = m_pEffect->SetFloatArray(m_TextureSizeHandle,(float*)x,2);
	hr = m_pEffect->SetFloatArray(m_OutputSizeHandle,(float*)y,2);
	hr = m_pEffect->SetFloatArray(m_FrameCount,(float*)frame_count,2);
