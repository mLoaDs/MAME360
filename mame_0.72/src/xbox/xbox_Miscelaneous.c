...
int osd_display_loading_rom_message( const char *name, struct rom_load_data *romdata )
{
  ...
  swprintf( memory, L"Available: %lu / Total: %lu", memStatus.dwAvailPhys, memStatus.dwTotalPhys );
	...
	BeginFontRender( TRUE, FONTTYPE_DEFAULT );
		#ifdef _DEBUG
			FontRender( 640, 100, D3DCOLOR_XRGB( 255, 255, 255 ), L"MAMEoX version " LVERSION_STRING, 2 );
		#endif
		FontRender( 640, 300, D3DCOLOR_XRGB( 255, 255, 255 ), title, 2 );
    FontRender( 640, 483, D3DCOLOR_XRGB( 255, 255, 255 ), L"Memory Status", 2 );
    FontRender( 640, 508, D3DCOLOR_XRGB( 255, 255, 255 ), memory, 2 );
  EndFontRender( FALSE );
...
void osd_print_error( const char *fmt, ... )
{
  ...
  mbstowcs( wBuf, buf, 1023 );
	...
	BeginFontRender( TRUE, FONTTYPE_DEFAULT );
    FontRender( 640, 50, D3DCOLOR_RGBA( 255, 255, 255, 255 ), L"Error:", 2 /*XBFONT_CENTER_X*/ );
	FontRender( 640, 75, D3DCOLOR_RGBA( 255, 255, 255, 255 ), wBuf, 2 /*XBFONT_CENTER_X*/ );
	FontRender( 640, 408, D3DCOLOR_RGBA( 255, 255, 255, 255), L"Press any button to continue.", 2 /*XBFONT_CENTER_X*/ );
	EndFontRender( TRUE );
